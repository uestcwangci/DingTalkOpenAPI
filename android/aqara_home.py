...
class CameraHelper:
    ...
    def keep_watch(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(yolo_detect, 30, 2)
            while loop:
                ...
                if future.done():
                    ...
                    if result:
                        ...
                        reply = self.openAI.chat_with_gpt("帮我看下我电脑在哪里？", img_url_list=full_urls)

                        self.dingtalk_api.send_update_request(content=self.dingtalk_api.build_request_content(
                                template_id="b9b2615c-a220-439d-a56b-74bb39baa618.schema",
                                card_data={"prompt": "帮我看下我电脑在哪里？", "reply": reply, "imgList": full_urls}))
